  ...
  return (
    <div className="min-h-screen space-y-6 bg-[#050b14] p-4 text-slate-100 selection:bg-sky-500/30 sm:p-6 lg:p-8">
      ...
      <div className="relative z-10 mx-auto max-w-[1600px] space-y-6">
        ...
        {activeTab === "resumen" ? <div className="grid grid-cols-1 gap-6 lg:grid-cols-2"><ShellCard><ShellCardHeader><ShellCardTitle>Top Lineas de Mayor Facturacion</ShellCardTitle><p className="mt-1 text-sm text-slate-400">Ingresos brutos generados en el periodo actual.</p></ShellCardHeader><ShellCardContent className="h-[400px]">{chartDataTop.length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={chartDataTop} layout="vertical" margin={{ left: 10, right: 88 }}><CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" horizontal={false} /><XAxis type="number" tickFormatter={formatCurrencyCompact} {...CHART_AXIS_STYLE} tick={CHART_TICK_STYLE} /><YAxis type="category" dataKey="linea" width={140} {...CHART_AXIS_STYLE} tick={CHART_TICK_STYLE} /><Tooltip cursor={{ fill: "rgba(255,255,255,0.05)" }} content={<CustomTooltip format="currency" />} /><Bar dataKey="ventasMonto" radius={[0, 6, 6, 0]} barSize={24}>{chartDataTop.map((entry, index) => <Cell key={entry.linea} fill={CHART_COLORS[index % CHART_COLORS.length]} />)}<LabelList position="right" formatter={(value) => formatCurrencyCompact(Number(value))} fill="#f8fafc" fontSize={12} fontWeight={800} /></Bar></BarChart></ResponsiveContainer> : <EmptyState />}</ShellCardContent></ShellCard><ShellCard><ShellCardHeader><ShellCardTitle>Evolucion YoY por Linea</ShellCardTitle><p className="mt-1 text-sm text-slate-400">Crecimiento porcentual comparado con el año anterior.</p></ShellCardHeader><ShellCardContent className="h-[400px]">{chartDataGrowth.length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={chartDataGrowth} layout="vertical" margin={{ left: 10, right: 88 }}><CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" horizontal={false} /><XAxis type="number" tickFormatter={(value) => `${value}%`} {...CHART_AXIS_STYLE} tick={CHART_TICK_STYLE} /><YAxis type="category" dataKey="linea" width={140} {...CHART_AXIS_STYLE} tick={CHART_TICK_STYLE} /><Tooltip cursor={{ fill: "rgba(255,255,255,0.05)" }} content={<CustomTooltip format="percent" dataKey="yoyGrowth" name="Crecimiento YoY" />} /><ReferenceLine x={0} stroke="#475569" strokeWidth={2} /><Bar dataKey="yoyGrowth" radius={6} barSize={24}>{chartDataGrowth.map((entry) => <Cell key={entry.linea} fill={(entry.yoyGrowth ?? 0) >= 0 ? "#10b981" : "#f43f5e"} />)}<LabelList position="right" formatter={(value) => formatPercent(Number(value))} fill="#f8fafc" fontSize={12} fontWeight={800} /></Bar></BarChart></ResponsiveContainer> : <EmptyState msg="Selecciona un año especifico para ver variacion YoY." />}</ShellCardContent></ShellCard></div> : null}
